...
class ManageEvent extends StatelessWidget {
  ...
  ManageEvent({Key? key, this.eventDetails, required this.uId})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
          onPressed: () {
            var result = addPerson(
                eventDetails!.id,
                eventDetails!.numberOfPeople,
                eventDetails!.price,
                eventDetails!.numberOfPeopleMax,
                uId);
            switch (result) {
              case (-1):
                showDialog(
              ...
              case (1):
                showDialog<String>(
                    ...
                    builder: (BuildContext context) => AlertDialog(
                          ...
                          content: const Text(
                              'Thank you for your registration. You can find the different information about your registration under the tab "My reservations" '),
                          actions: <Widget>[
                            TextButton(
                              onPressed: () => Navigator.pop(context, 'OK'),
                              child: const Text('OK'),
  ...
  int addPerson(
      String idEvent, int people, double price, int peopleMax, String? idUser) {
    if (people >= peopleMax) {
      return -1;
    } else {
